...
func CheckProduct(product_id int) (bool, string, error) {
	...
	err := database.DB.Raw("select count(*) from products where id = ?", product_id).Scan(&count).Error
...
func QuantityOfProductInCart(userId int, productId int) (int, error) {
	var productQty int
	err := database.DB.Raw("select quantity from carts where user_id = ? and product_id = ?", userId, productId).Scan(&productQty).Error
	if err != nil {
		return 0, err
	}
...
func AddItemIntoCart(userId int, productId int, Quantity int, productprice float64) error {
	if err := database.DB.Exec("insert into carts (user_id,product_id,quantity,total_price) values(?,?,?,?)", userId, productId, Quantity, productprice).Error; err != nil {
		return err
	}
	return nil
...
func TotalPriceForProductInCart(userID int, productID int) (float64, error) {
	...
	if err := database.DB.Raw("select sum(total_price) as total_price from carts where user_id = ? and product_id = ?", userID, productID).Scan(&totalPrice).Error; err != nil {
		return 0.0, err
	}
...
func UpdateCart(quantity int, price float64, userID int, product_id int) error {
	...
	if err := database.DB.Exec("update carts set quantity = ?, total_price = ? where user_id = ? and product_id = ?", quantity, price, userID, product_id).Error; err != nil {
		return err
	}
